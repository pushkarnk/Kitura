...
public struct CodableHelpers {
     ...
     */
    public static func httpStatusCode(from error: RequestError) -> HTTPResponseStatus {
        // ORM status code 7XX mapped to internalServerError 500
        if error.httpCode >= 700 && error.httpCode < 800 { return HTTPResponseStatus.internalServerError }
        return HTTPResponseStatus(statusCode: error.rawValue) //?? HTTPResponseStatus.custom(code: 0, reasonPhrase: "unknown")
    }
     ...
     *            it will be encoded and sent as the body of the response.
     */
    public static func constructOutResultHandler<OutputType: Codable>(successStatus: HTTPResponseStatus = .ok, response: RouterResponse, completion: @escaping () -> Void) -> CodableResultClosure<OutputType> {
     ...
     *            it will be encoded and sent as the body of the response.
     */
    public static func constructTupleArrayOutResultHandler<Id: Identifier, OutputType: Codable>(successStatus: HTTPResponseStatus = .ok, response: RouterResponse, completion: @escaping () -> Void) -> IdentifierCodableArrayResultClosure<Id, OutputType> {
        return { codableOutput, error in
            if let error = error {
                response.status(httpStatusCode(from: error))
                do {
     ...
     *            it will be encoded and sent as the body of the response.
     */
    public static func constructIdentOutResultHandler<IdType: Identifier, OutputType: Codable>(successStatus: HTTPResponseStatus = .ok, response: RouterResponse, completion: @escaping () -> Void) -> IdentifierCodableResultClosure<IdType, OutputType> {
